import type { RaidItem } from "../../types";

// Armored Double Door is mechanically identical to the Armored Door in Rust:
// same 1000 HP and the same per-explosive damage, so the tables are the same.
// This file previously held a verbatim copy of the Tool Cupboard data (100 HP),
// which made the calculator quote 1 C4 / 1 rocket / 9 explosive ammo for it.
export const RaidDataArmoredDoubleDoor: RaidItem[] = [
  {
    name: "Torpedo",
    category: "torpedo",
    side: "both",
    damage: 8,
    quantity: 125,
    time: "12 min 28 sec",
  },
  {
    name: "Catapult - Propane Explosive Bomb",
    category: "siege weapons",
    side: "both",
    damage: 220.375,
    quantity: 5,
    time: "12 sec",
  },
  {
    name: "Assault Rifle - Explosive 5.56 Rifle Ammo",
    category: "guns",
    side: "both",
    damage: 4.008,
    quantity: 250,
    time: "1 min 7 sec",
  },
  {
    name: "Bolt Action Rifle - Explosive 5.56 Rifle Ammo",
    category: "guns",
    side: "both",
    damage: 4.0128,
    quantity: 250,
    time: "10 min 27 sec",
  },
  {
    name: "High Caliber Revolver - Explosive 5.56 Rifle Ammo",
    category: "guns",
    side: "both",
    damage: 4.0104,
    quantity: 250,
    time: "4 min 49 sec",
  },
  {
    name: "HMLMG - Explosive 5.56 Rifle Ammo",
    category: "guns",
    side: "both",
    damage: 4.0092,
    quantity: 250,
    time: "56 sec",
  },
  {
    name: "L96 Rifle - Explosive 5.56 Rifle Ammo",
    category: "guns",
    side: "both",
    damage: 4.0128,
    quantity: 250,
    time: "11 min 7 sec",
  },
  {
    name: "LR-300 Assault Rifle - Explosive 5.56 Rifle Ammo",
    category: "guns",
    side: "both",
    damage: 4.0064,
    quantity: 250,
    time: "1 min",
  },
  {
    name: "M16A2 - Explosive 5.56 Rifle Ammo",
    category: "guns",
    side: "both",
    damage: 4.0056,
    quantity: 250,
    time: "56 sec",
  },
  {
    name: "M249 - Explosive 5.56 Rifle Ammo",
    category: "guns",
    side: "both",
    damage: 4.0104,
    quantity: 250,
    time: "44 sec",
  },
  {
    name: "M39 Rifle - Explosive 5.56 Rifle Ammo",
    category: "guns",
    side: "both",
    damage: 4.008,
    quantity: 250,
    time: "1 min 20 sec",
  },
  {
    name: "Explosive 5.56 Rifle Ammo - Semi-Automatic Rifle",
    category: "explosive",
    side: "both",
    damage: 4.0064,
    quantity: 250,
    time: "1 min 46 sec",
  },
  {
    name: "Semi-Automatic Rifle - Explosive 5.56 Rifle Ammo",
    category: "guns",
    side: "both",
    damage: 4.0064,
    quantity: 250,
    time: "1 min 46 sec",
  },
  {
    name: "SKS - Explosive 5.56 Rifle Ammo",
    category: "guns",
    side: "both",
    damage: 4.0068,
    quantity: 250,
    time: "1 min 45 sec",
  },
  {
    name: "Mortar Shell",
    category: "explosive",
    side: "both",
    damage: 48.375,
    quantity: 21,
    time: "53 sec",
  },
  {
    name: "Timed Explosive Charge",
    category: "explosive",
    side: "both",
    damage: 440,
    quantity: 3,
    time: "13 sec",
  },
  {
    name: "Rocket",
    category: "explosive",
    side: "both",
    damage: 220.375,
    quantity: 5,
    time: "24 sec",
  },
  {
    name: "Propane Explosive Bomb - Planting",
    category: "explosive",
    side: "both",
    damage: 128.375,
    quantity: 8,
    time: "6 sec",
  },
  {
    name: "Satchel Charge",
    category: "explosive",
    side: "both",
    damage: 70,
    quantity: 15,
    time: "30 sec",
  },
  {
    name: "Beancan Grenade - Stuck (right click)",
    category: "explosive",
    side: "both",
    damage: 14.5,
    quantity: 69,
    time: "4 min 21 sec",
  },
  {
    name: "High Velocity Rocket",
    category: "explosive",
    side: "both",
    damage: 24,
    quantity: 42,
    time: "4 min 12 sec",
  },
  {
    name: "F1 Grenade - Stuck (right click)",
    category: "explosive",
    side: "both",
    damage: 5,
    quantity: 200,
    time: "8 min 21 sec",
  },
  {
    name: "Cannonball",
    category: "explosive",
    side: "both",
    damage: 0.5,
    quantity: 2000,
    time: "1 hour 39 min 57 sec",
  },
  {
    name: "Fragmentation Mortar Shell",
    category: "explosive",
    side: "both",
    damage: 0.375,
    quantity: 2667,
    time: "1 hour 58 min 30 sec",
  },
  {
    name: "MLRS Rocket",
    category: "explosive",
    side: "both",
    damage: 280.375,
    quantity: 4,
    time: "0 sec",
  },
  {
    name: "Mounted Ballista - Hammerhead Bolt",
    category: "siege weapons",
    side: "both",
    damage: 20,
    quantity: 50,
    time: "3 min 16 sec",
  },
  {
    name: "Ballista - Hammerhead Bolt",
    category: "siege weapons",
    side: "both",
    damage: 20,
    quantity: 50,
    time: "3 min 16 sec",
  },
  {
    name: "Battering Ram",
    category: "siege weapons",
    side: "both",
    damage: 80.25,
    quantity: 13,
    time: "58 sec",
  },
  {
    name: "Catapult - Scattershot",
    category: "siege weapons",
    side: "both",
    damage: 0.4,
    quantity: 2500,
    time: "2 hours 4 min 57 sec",
  },
  {
    name: "Catapult - Bee Catapult Bomb",
    category: "siege weapons",
    side: "both",
    damage: 0.1,
    quantity: 10000,
    time: "8 hours 19 min 57 sec",
  },
  {
    name: "40mm HE Grenade",
    category: "explosive",
    side: "both",
    damage: 28.275,
    quantity: 36,
    time: "42 sec",
  },
  {
    name: "Apple Pie - Throw",
    category: "throw",
    side: "both",
    damage: 0.05,
    quantity: 1,
    time: "8 hours 20 min",
  },
  {
    name: "Bear Pie - Throw",
    category: "throw",
    side: "both",
    damage: 0.05,
    quantity: 1,
    time: "8 hours 20 min",
  },
  {
    name: "Big Cat Pie - Throw",
    category: "throw",
    side: "both",
    damage: 0.05,
    quantity: 1,
    time: "8 hours 20 min",
  },
  {
    name: "Chicken Pie - Throw",
    category: "throw",
    side: "both",
    damage: 0.05,
    quantity: 1,
    time: "8 hours 20 min",
  },
  {
    name: "Crocodile Pie - Throw",
    category: "throw",
    side: "both",
    damage: 0.05,
    quantity: 1,
    time: "8 hours 20 min",
  },
  {
    name: "Fish Pie - Throw",
    category: "throw",
    side: "both",
    damage: 0.05,
    quantity: 1,
    time: "8 hours 20 min",
  },
  {
    name: "Hunters Pie - Throw",
    category: "throw",
    side: "both",
    damage: 0.05,
    quantity: 1,
    time: "8 hours 20 min",
  },
  {
    name: "Pork Pie - Throw",
    category: "throw",
    side: "both",
    damage: 0.05,
    quantity: 1,
    time: "8 hours 20 min",
  },
  {
    name: "Pumpkin Pie - Throw",
    category: "throw",
    side: "both",
    damage: 0.05,
    quantity: 1,
    time: "8 hours 20 min",
  },
  {
    name: "Survivor's Pie - Throw",
    category: "throw",
    side: "both",
    damage: 0.05,
    quantity: 1,
    time: "8 hours 20 min",
  },
  {
    name: "Candy Cane Club",
    category: "melee",
    side: "both",
    damage: 0.09,
    quantity: 506,
    time: "3 hours 13 min 37 sec",
  },
  {
    name: "Candy Cane Club - Throw",
    category: "throw",
    side: "both",
    damage: 0.06,
    quantity: 667,
    time: "6 hours 56 min 40 sec",
  },
  {
    name: "Snowball - Throw",
    category: "throw",
    side: "both",
    damage: 0.125,
    quantity: 8000,
    time: "3 hours 20 min",
  },
  {
    name: "Snowball Gun - Snowball",
    category: "guns",
    side: "both",
    damage: 0.04,
    quantity: 500,
    time: "1 hour 9 min 34 sec",
  },
  {
    name: "Flashlight",
    category: "melee",
    side: "both",
    damage: 0.075,
    quantity: 1026,
    time: "3 hours 59 min 19 sec",
  },
  {
    name: "Shovel",
    category: "melee",
    side: "both",
    damage: 0.025,
    quantity: 785,
    time: "5 hours 46 min 24 sec",
  },
  {
    name: "Bee Grenade - Stuck (right click)",
    category: "explosive",
    side: "both",
    damage: 0.5,
    quantity: 2000,
    time: "5 hours 33 min 27 sec",
  },
  {
    name: "Bone Club",
    category: "melee",
    side: "both",
    damage: 0.06,
    quantity: 695,
    time: "3 hours 26 min",
  },
  {
    name: "Bone Club - Throw",
    category: "throw",
    side: "both",
    damage: 0.06,
    quantity: 834,
    time: "6 hours 56 min 40 sec",
  },
  {
    name: "Boomerang",
    category: "melee",
    side: "both",
    damage: 0.025,
    quantity: 2353,
    time: "12 hours 52 min 32 sec",
  },
  {
    name: "Boomerang - Throw",
    category: "throw",
    side: "both",
    damage: 0.225,
    quantity: 445,
    time: "1 hour 51 min 7 sec",
  },
  {
    name: "Birthday Cake",
    category: "melee",
    side: "both",
    damage: 0.075,
    quantity: 4445,
    time: "6 hours 2 min 58 sec",
  },
  {
    name: "Birthday Cake - Throw",
    category: "throw",
    side: "both",
    damage: 0.15,
    quantity: 1,
    time: "2 hours 46 min 40 sec",
  },
  {
    name: "Hatchet",
    category: "melee",
    side: "both",
    damage: 0.05,
    quantity: 299,
    time: "5 hours 4 min 58 sec",
  },
  {
    name: "Mace",
    category: "melee",
    side: "both",
    damage: 0.3,
    quantity: 209,
    time: "1 hour 54 min 36 sec",
  },
  {
    name: "Mace - Throw",
    category: "throw",
    side: "both",
    damage: 0.15,
    quantity: 134,
    time: "2 hours 46 min 40 sec",
  },
  {
    name: "Paddle",
    category: "melee",
    side: "both",
    damage: 0.15,
    quantity: 152,
    time: "2 hours 21 min 24 sec",
  },
  {
    name: "Rock",
    category: "melee",
    side: "both",
    damage: 0.05,
    quantity: 589,
    time: "7 hours 23 min 8 sec",
  },
  {
    name: "Rock - Throw",
    category: "throw",
    side: "both",
    damage: 0.05,
    quantity: 800,
    time: "8 hours 20 min",
  },
  {
    name: "Salvaged Hammer",
    category: "melee",
    side: "both",
    damage: 0.2,
    quantity: 90,
    time: "1 hour 24 min 49 sec",
  },
  {
    name: "Salvaged Hammer - Throw",
    category: "throw",
    side: "both",
    damage: 0.1,
    quantity: 80,
    time: "4 hours 10 min",
  },
  {
    name: "Stone Hatchet",
    category: "melee",
    side: "both",
    damage: 0.025,
    quantity: 1539,
    time: "10 hours 25 min 38 sec",
  },
  {
    name: "Stone Pickaxe",
    category: "melee",
    side: "both",
    damage: 0.035,
    quantity: 1243,
    time: "7 hours 29 min 16 sec",
  },
  {
    name: "Torch",
    category: "melee",
    side: "both",
    damage: 0.025,
    quantity: 1539,
    time: "14 hours 52 min 18 sec",
  },
  {
    name: "Torch - Lit",
    category: "melee",
    side: "both",
    damage: 0.025,
    quantity: 5715,
    time: "16 hours 1 min 54 sec",
  },
];
