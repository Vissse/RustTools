...
function SalvagingCard({ target, data, idx }: { target: SalvagingTarget, data: SalvagingData[], idx: number }) {
    ...
    <div className="w-full max-w-[1400px] mx-auto px-6 py-20 text-text font-sans" {...tooltip}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {TARGETS.map((target, idx) => (
          <SalvagingCard key={target} target={target} data={SALVAGING_DATA[target]} idx={idx} />
        ))}
      </div>
    </div>
  )
