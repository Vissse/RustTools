...
import { MONUMENT_SLUGS, getMonumentBySlug } from '@/lib/data/monuments-data/slugs'
...
export function generateStaticParams() {
  return MONUMENT_SLUGS.map((slug) => ({ slug }))
}

export async function generateMetadata(props: Props): Promise<Metadata> {
  const params = await props.params
  const monument = getMonumentBySlug(params.slug)
  if (!monument) return { title: 'Not Found' }

  return seoMetadata({
    title: `${monument.name} — Rust Monument`,
    description: monument.description || `Everything about ${monument.name} in Rust: loot, puzzles, keycards, scientists, radiation and more.`,
    path: `/world/monuments/${params.slug}`,
  })
}
    ...
    <>
      <JsonLd
        data={breadcrumbJsonLd([
          ...
          { name: 'Monuments', path: '/world/monuments' },
          { name: monument.name, path: `/world/monuments/${params.slug}` },
        ])}
      />
          ...
          <div className="relative z-10 w-full px-6 md:px-10 pb-10 flex flex-col md:flex-row justify-between md:items-end gap-6">
            <div>
              ...
              <div className="text-sm font-display uppercase text-text-dim mb-6 flex items-center space-x-2 tracking-widest">
                ...
                <span>/</span>
                <Link href="/world/monuments" className="hover:text-white transition-colors">Monuments</Link>
                <span>/</span>
        ...
        <div className="w-full px-6 md:px-10 space-y-12 max-w-7xl mx-auto">
          ...
          <div className="w-full space-y-12">
            ...
            <section className="bg-panel border border-white/5 p-8 rounded-2xl shadow-xl">
              ...
              <div className="aspect-video w-full bg-panel-2 border border-white/10 rounded-xl flex items-center justify-center relative overflow-hidden">
